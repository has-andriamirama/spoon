...
export async function sendReservationPaymentLink(data: {
  ...
  return send({
    from: FROM_EMAIL,
    to: data.guestEmail,
    subject: `Votre réservation Spoon — paiement de l'acompte`,
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; background: #0A0A0A; color: #F5F0EB; padding: 40px; border-radius: 12px;">
        ...
        <h2 style="font-size: 20px; margin-bottom: 20px;">Votre réservation est pré-enregistrée</h2>
        <p>Bonjour ${data.guestFirstName},</p>
        <p>
          Notre équipe a enregistré votre réservation chez Spoon.
          Pour la valider, merci de régler l'acompte demandé via le bouton ci-dessous.
        </p>

        <div style="background: #141414; border: 1px solid #222; border-radius: 8px; padding: 24px; margin: 24px 0;">
          <h3 style="color: #C8973A; margin: 0 0 16px;">Détails</h3>
          <p><strong>Date :</strong> ${formatDate(data.date)}</p>
          <p><strong>Heure :</strong> ${data.timeSlot}</p>
          <p><strong>Couverts :</strong> ${data.covers}</p>
          <p><strong>Acompte :</strong> ${formatPrice(data.amount)}</p>
          <p><strong>Référence :</strong> #${data.reservationId.slice(-8).toUpperCase()}</p>
        </div>

        <div style="text-align: center; margin: 32px 0;">
          <a href="${data.checkoutUrl}" style="background: #C8973A; color: #0A0A0A; padding: 14px 28px; border-radius: 8px; text-decoration: none; font-weight: bold; display: inline-block;">
            Payer mon acompte
          </a>
        </div>

        <p style="color: #9A8F84; font-size: 13px; line-height: 1.6;">
          Ce lien de paiement est valable 24 heures, jusqu'au
          ${data.expiresAt.toLocaleString("fr-FR", { dateStyle: "short", timeStyle: "short", timeZone: "Indian/Reunion" })}.
          En cas d'échec de carte, vous pouvez réessayer tant que le lien est valide.
          À expiration, la réservation est automatiquement annulée.
        </p>

        <p style="color: #9A8F84; font-size: 13px;">
