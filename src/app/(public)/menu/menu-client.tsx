...
export default function MenuClientPage({ categories, dishes }: Props) {
  ...
  const [selectedDish, setSelectedDish] = useState<(Dish & { category: MenuCategory }) | null>(null);
  ...
  return (
    <div className="min-h-screen pt-24 pb-20">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-12">
        <p className="text-[#C8973A] text-sm font-medium uppercase tracking-widest mb-3">Gastronomie créole</p>
        <h1 className="font-display text-5xl text-[#F5F0EB] mb-4">La Carte</h1>
        <p className="text-[#9A8F84] max-w-lg">Des saveurs authentiques de l&apos;île, revisitées avec créativité et passion.</p>
      </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          ...
          <div className="flex items-center gap-2 overflow-x-auto pb-2 scrollbar-hide mb-3">
            <button
              ...
              className={cn("shrink-0 px-4 py-2 rounded-full text-sm font-medium transition-colors", activeCategory === "all" ? "bg-[#C8973A] text-[#0A0A0A]" : "bg-[#141414] text-[#9A8F84] hover:text-[#F5F0EB] border border-[#222]")}
            >
              Tout
            </button>
            {categories.map((cat) => (
              <button
                ...
                className={cn("shrink-0 px-4 py-2 rounded-full text-sm font-medium transition-colors", activeCategory === cat.id ? "bg-[#C8973A] text-[#0A0A0A]" : "bg-[#141414] text-[#9A8F84] hover:text-[#F5F0EB] border border-[#222]")}
              >
                {cat.name}
              </button>
            ))}
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {DIETARY_TAGS.slice(0, 3).map((tag) => (
              <button
                ...
                className={cn("flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium transition-colors border", activeDietary.includes(tag.id) ? "bg-[#C8973A]/20 border-[#C8973A]/50 text-[#C8973A]" : "bg-[#141414] border-[#222] text-[#5A5249] hover:text-[#9A8F84]")}
              >
              ...
              >
                <div className="aspect-[4/3] relative overflow-hidden bg-[#1a1a1a]">
                  {dish.imageUrl ? (
                    ...
                    <div className="absolute inset-0 flex items-center justify-center text-[#333]"><Flame size={40} /></div>
                  )}
                  {dish.isDailySpecial && (
                    <div className="absolute top-2 left-2">
                      <span className="bg-[#C8973A] text-[#0A0A0A] text-xs font-bold px-2 py-0.5 rounded-full">✦ Spécialité</span>
                    </div>
                  )}
                </div>
                <div className="p-4">
                  <div className="flex items-start justify-between gap-2 mb-1">
                    <h3 className="font-display text-base text-[#F5F0EB] font-semibold leading-tight">{dish.name}</h3>
                    <span className="text-[#C8973A] font-semibold text-sm shrink-0">{formatPrice(dish.price)}</span>
                  </div>
                  <p className="text-xs text-[#9A8F84] leading-relaxed line-clamp-2 mb-3">{dish.description}</p>
                  <div className="flex items-center gap-1.5 flex-wrap">
                    {dish.dietaryTags.slice(0, 2).map((tag) => {
                      const t = DIETARY_TAGS.find(dt => dt.id === tag);
                      return t ? <Badge key={tag} variant="gold" className="text-[10px]">{t.label}</Badge> : null;
                    })}
                    {dish.allergens.length > 0 && (
                      <span className="text-[10px] text-[#5A5249]">{dish.allergens.length} allergène{dish.allergens.length > 1 ? "s" : ""}</span>
                    )}
                  </div>
                </div>
              </button>
      ...
      {/* Dish detail modal */}
      {selectedDish && (
        <Modal open={!!selectedDish} onClose={() => setSelectedDish(null)} title={selectedDish.name} description={selectedDish.category.name}>
          {selectedDish.imageUrl && (
            <div className="aspect-video relative rounded-lg overflow-hidden mb-4 -mt-2">
              <Image src={selectedDish.imageUrl} alt={selectedDish.name} fill className="object-cover" />
            </div>
          )}
          <p className="text-[#9A8F84] text-sm leading-relaxed mb-4">{selectedDish.description}</p>
          <div className="flex items-center justify-between mb-4">
            <span className="text-[#C8973A] text-2xl font-display font-semibold">{formatPrice(selectedDish.price)}</span>
            ...
            <div className="mb-4">
              ...
              <div className="flex flex-wrap gap-2">
                {selectedDish.dietaryTags.map(tag => {
                  const t = DIETARY_TAGS.find(dt => dt.id === tag);
                  return t ? <Badge key={tag} variant="green">{t.label}</Badge> : null;
                })}
              </div>
            </div>
          )}
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {selectedDish.allergens.map(a => {
                  ...
                  return allergen ? <Badge key={a} variant="yellow">{allergen.label}</Badge> : null;
                })}
              </div>
            </div>
          )}
