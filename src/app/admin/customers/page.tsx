		...
		<div>
			<div className="flex items-center justify-between mb-6">
				<h1 className="font-display text-3xl text-[#F5F0EB]">
					Clients <span className="text-[#5A5249] text-xl">({customers.length})</span>
				</h1>
			</div>
			<form className="mb-6">
				<input
					...
					className="h-10 w-full max-w-sm px-4 rounded-lg bg-[#141414] border border-[#222] text-sm text-[#F5F0EB] placeholder:text-[#5A5249] focus:border-[#C8973A] focus:outline-none"
				/>
			</form>
			<div className="bg-[#141414] border border-[#222] rounded-xl overflow-hidden">
				{customers.length === 0 ? (
					<div className="text-center py-16">
						<Users size={40} className="text-[#333] mx-auto mb-4" />
						<p className="text-[#5A5249]">Aucun client trouvé</p>
					</div>
				) : (
					<table className="w-full">
						<thead>
							<tr className="border-b border-[#222]">
								{["Client", "Email", "Téléphone", "Réservations", "Inscrit le", ""].map((h) => (
									<th
										key={h}
